...
static BASE_PENALTY: KeyMap<f64> = KeyMap([
  6.0, 5.5, 4.0, 4.0, 4.5, 4.5, // left number
  5.5, 4.5, 4.0, 4.0, 5.0, 5.5, 6.0, // right number
  3.0, 1.0, 1.0, 1.5, 3.0, // left top
  3.0, 1.5, 1.0, 1.0, 3.0, 4.0, // right top
  0.5, 0.5, 0.0, 0.0, 1.5, // left mid
  1.5, 0.0, 0.0, 0.5, 0.5, 2.0, // right mid
  2.0, 2.0, 1.5, 1.5, 2.5, // left bottom
  2.5, 1.5, 1.5, 2.0, 2.0, //right bottom
  0.0, 0.0, //thumbs
]);
...
fn penalize<'a, 'b>(
  ...
  if curr.hand == old1.hand {
    ...
    if curr.finger == old1.finger && curr.pos != old1.pos {
      let penalty = 5.0 + if curr.center { 5.0 } else { 0.0 } + if old1.center { 5.0 } else { 0.0 };
